...
import { CouncilType, RegistrationPeriodType, TopicParamType, TopicType } from '@assets/interface';
...
const CounterPage = ({ params: { lng } }: PageProps) => {
    ...
    const councilQuery = useQuery<CouncilType[], AxiosError<ResponseType>>({
        ...
        queryKey: ['councils', 'list'],
        queryFn: async () => {
            const response = await request.get<CouncilType[]>(`${API.admin.council}`);
            ...
            <div className='flex align-items-center justify-content-center gap-3'>
                {permission.create && (
                    <i
                        ...
                        onClick={() =>
                            router.push(language.addPrefixLanguage(lng, `${ROUTES.thesis.council_topic}/${data.id}`))
                        }
                    />
                )}
            </div>
        ...
        <div className='flex flex-column gap-4'>
            ...
            <div className='flex align-items-center'>
                ...
                <Dropdown
                    ...
                    options={councilQuery.data?.map((t) => ({
                        label: t.name,
                        value: t.id,
                    }))}
                    ...
                    onChange={(e) => {
                        ...
                        setParams((prev) => ({
                            ...prev,
                            filters: request.handleFilter(prev.filters, 'councilId', '==', e),
                        }));
                    }}
                />
            </div>
